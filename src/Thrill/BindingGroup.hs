module Thrill.BindingGroup where

import           Thrill.Prelude

import           Thrill.Syntax.Expression
import           Thrill.Syntax.Pattern

import           Thrill.Syntax
import           Thrill.Error as Thrill
...
bindingGroups :: (MonadError (Thrill.Error b) m) => Module Name a -> m (Module' (BoundModules Name a))
...
sortedClassDeclarations :: (MonadError (Thrill.Error b) m) => [Decl Name a] -> m [BindingGroup Name a]
...
sortedInstances :: (MonadError (Thrill.Error b) m) => [Decl Name a] -> [Decl Name a] -> m [BindingGroup Name a]
